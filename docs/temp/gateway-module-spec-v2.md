...
```
Gateway permit-paths:      /api/main-service/auth/login  （外部路径）
main-service permit-paths: /auth/login                    （StripPrefix 后路径）
```
...
| GW-C-03 | C-42 对齐                   | Gateway 配外部路径，下游配 Strip 后路径                                                          |
...
| #      | 事项                                            | 影响模块      | 优先级 |
|--------|-----------------------------------------------|-----------|-----|
| GW-Q-6 | 生产前端域名（`gateway.cors.allowed-origins` prod 值） | GW-4、GW-5 | 低   |
| GW-Q-8 | main-service Controller 路径确认，保证 C-42 双端对齐     | GW-1      | 中   |
