...
> 版本：v5.0 | 所属项目：com.jugu.propertylease / security-starter
...
>
> **v5.0 变更：**
> - C-40 已解决：mustache 模板 vendorExtensions key 格式确认为
    `vendorExtensions.x-required-permission`
> - C-43 已解决：`DefaultConfigurationCustomizer` 准确 API 已封装至 `common` 模块 `JooqConfigSupport`
> - 补充 security.mode=mock 与 openapi-generator 联动说明
> - 补充 /internal/v1/** 放行路径约定
...
字段：
  ...
  List<String> permitPaths    可选，默认空列表
    ...
    C-42 放行路径双端对齐约定：
      Gateway 配外部路径（StripPrefix 前）：/api/main-service/auth/**
      微服务配 StripPrefix 后路径：          /auth/**、/internal/v1/**
      两端均不携带 Service JWT，微服务侧若不放行则返回 401

    main-service 典型配置：
      permit-paths:
        - /auth/**          # 登录/绑定/刷新/登出，对应 Gateway /api/main-service/auth/**
        - /internal/v1/**   # 内部接口（服务间直调，不经 Gateway），Service JWT 认证后
                            # 在 Service 层校验调用方白名单
...
**mock 模式与 openapi-generator 联动：**

mock 模式下 `@PreAuthorize` 注解正常生效，意味着 mustache 模板生成的权限鉴权代码在本地
开发环境中也可以测试。通过配置不同的 `mock-user.permissions` 可以模拟有权限/无权限两种场景：
...
filter(ServerWebExchange exchange, WebFilterChain chain)：
  ...
  3. 认证路径：
     ...
     - Mono.fromCallable(() -> parseUserToken)
       User JWT 由 main-service IAM 模块的 UserJwtIssuer 签发（使用 jwt.user.secret）
       Gateway 使用相同 jwt.user.secret 验证，两端必须配置相同密钥
...
> **User JWT 签发说明：**
> User JWT 由 main-service IAM 模块的 `UserJwtIssuer` 签发，使用 `security.jwt.user.secret`。
> `UserJwtIssuer` 是 main-service 内部类，**不是** security-starter 提供的组件。
> security-starter 提供的 `ServiceTokenGenerator` 只用于生成 Service JWT。
> Gateway 的 `ReactiveUserJwtFilter` 使用 `JwtTokenParser.parseUserToken()` 验证 User JWT。
...
规范与 v4.0 一致。补充 JooqConfig 注册方式（C-43 已解决）：

```java
// 各微服务 config/JooqConfig.java
// 使用 common 模块 JooqConfigSupport 工具类注册 VisitListener
...
public class JooqConfig {
    ...
    public DefaultConfigurationCustomizer jooqCustomizer(
        ...
        return JooqConfigSupport.withVisitListener(listener);  // common 模块提供
    }
}
```

DataPermissionVisitListener 实现要点（C-32 待确认辅助方法设计）：

- 继承 security-starter 的 `DataPermissionVisitListener` 抽象类
- 实现 `queryInvolvesTable()` 判断当前 SQL 是否需要注入数据权限条件
- 仅 STAFF 类型用户需要数据权限过滤；ADMIN 全量访问（代码特判）；TENANT 只看自身数据（业务层过滤）
...
| #    | 事项                                                                      | 影响模块       | 优先级 | 状态                                                                |
|------|-------------------------------------------------------------------------|------------|-----|-------------------------------------------------------------------|
| C-31 | @AuditLog 的 beforeJson 获取方式（AOP 自动查 DB？手动传入？SpEL？）                      | 模块 7       | 低   | 待确认                                                               |
| C-32 | DataPermissionVisitListener.queryInvolvesTable() 辅助方法设计                 | 模块 6       | 低   | 待确认                                                               |
| C-40 | mustache 模板 vendorExtensions key 格式                                     | 外部模板       | 高   | ✅ 已解决：`vendorExtensions.x-required-permission`（连字符原样保留）           |
| C-43 | DefaultConfigurationCustomizer API 在 Spring Boot 3.2 + jOOQ 3.19 中的准确包名 | JooqConfig | 中   | ✅ 已解决：封装至 `common` 模块 `JooqConfigSupport`，见 common-module-spec-v2 |
